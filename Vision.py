...
class vision:
    ...
    def _calculate_rescale_percentage(self, new_width, new_height, original_width=720,
                                      original_height=400):  # original_width & original_width from Cropped template original dim
    ...
    def __init__(self, template_path, new_width, new_height, method=None):
        ...
        try:
            ...
            exit(0)
        width_scale, height_scale = self._calculate_rescale_percentage(new_width,
                                                                       new_height)  # Need to get Curr Windows Size
    ...
    def update(self, screenshot, hsv_values=None):
        self._lock.acquire()
        if self._debug_mode:
            self._screenshot = self._hsv_filter.applyHSVFilter(screenshot)
        else:
            self._screenshot = self._hsv_filter.applyHSVFilter(screenshot, hsv_values)  # Apply Best Values
        self._lock.release()
